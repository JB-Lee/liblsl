...
TEMPLATE_TEST_CASE("pushpull", "[basic][throughput]", char, double, std::string) {
	...
	for (auto nchan : param_nchan) {
		...
		std::list<std::unique_ptr<lsl::stream_inlet>> inlet_list;
		...
		for (auto n_inlets : param_inlets) {
			...
			for (std::size_t i=0; i < n_inlets; ++i) {
				auto inlet = std::make_unique<lsl::stream_inlet>(found_stream_info[0], 300, false);
				inlet->open_stream(.5);
				inlet_list.push_back(std::move(inlet));
			}
